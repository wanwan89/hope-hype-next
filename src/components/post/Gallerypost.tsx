...
export default function Gallerypost() {
  ...
  const fetchPosts = async (category = "all") => {
    ...
    try {
      ...
      if (category === "all" || !category) fetchedPosts = fetchedPosts.sort(() => Math.random() - 0.5);
      ...
      if (fetchedPosts.length > 0) {
        ...
        likesRes.data?.forEach(l => newCounts[l.post_id].likes++);
        commentsRes.data?.forEach(c => newCounts[c.post_id].comments++);
        repostsRes.data?.forEach(r => newCounts[r.post_id].reposts++);
        setCounts(newCounts);

        if (currentUser) {
          const { data: myLikes } = await supabase.from("likes").select("post_id").eq("user_id", currentUser.id).in("post_id", postIds);
          setMyLikedPosts(new Set(myLikes?.map(l => String(l.post_id))));

          const { data: myReposts } = await supabase.from("reposts").select("post_id").eq("user_id", currentUser.id).in("post_id", postIds);
  ...
  const handleLike = async (postId: string, creatorId: string) => {
    ...
    setCounts(prev => ({
      ...prev,
      [postId]: { ...prev[postId], likes: Math.max(0, prev[postId].likes + (isLiked ? -1 : 1)) }
    }));
    ...
    if (isLiked) {
      ...
      if (creatorId !== currentUser.id) {
        ...
        await supabase.from("notifications").insert({
          user_id: creatorId, actor_id: currentUser.id, post_id: postId, type: "like",
  ...
  const handleRepost = async (postId: string) => {
    ...
    setCounts(prev => ({
      ...prev,
      [postId]: { ...prev[postId], reposts: Math.max(0, prev[postId].reposts + (isReposted ? -1 : 1)) }
    }));

    if (isReposted) {
      await supabase.from("reposts").delete().match({ post_id: postId, user_id: currentUser.id });
    } else {
          ...
          posts.map(post => {
            ...
            return (
              <div key={post.id} className="card" style={!post.image_url ? { padding: '16px' } : {}}>
                {/* JSX Render Image / Text based on post.image_url */}
                {post.image_url && post.image_url.trim() !== "" ? (
                  <>
                    <div className="slider">
                      <div style={{ position: 'absolute', top: '12px', right: '12px', zIndex: 20 }}>{getMusicHtml(post)}</div>
                      <img src={post.image_url} className="active" loading="lazy" alt="Post" onClick={() => (window as any).openBigImage && (window as any).openBigImage(post.image_url)} />
                    ...
                    <div className="overlay">
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', width: '100%' }}>
                        ...
                        <button className="options-btn" onClick={() => (window as any).openPostOptions && (window as any).openPostOptions(post.id, isOwner, post.creator_id)} style={{ background: 'none', border: 'none', color: 'var(--text-muted)', cursor: 'pointer', padding: '5px' }}>
                  ...
                  <>
                     <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '12px' }}>
                      <div style={{ display: 'flex', gap: '12px', cursor: 'pointer' }} onClick={() => window.location.href=`/data?id=${post.creator_id}`}>
                        <img src={avatarUrl} style={{ width: '42px', height: '42px', borderRadius: '50%', objectFit: 'cover', border: '1px solid rgba(255,255,255,0.1)' }} />
                      ...
                      <button onClick={() => (window as any).openPostOptions && (window as any).openPostOptions(post.id, isOwner, post.creator_id)} style={{ background: 'none', border: 'none', color: 'var(--text-muted)', cursor: 'pointer', padding: '4px' }}>
                        <svg viewBox="0 0 24 24" width="20" height="20" fill="currentColor"><circle cx="12" cy="5" r="2"/><circle cx="12" cy="12" r="2"/><circle cx="12" cy="19" r="2"/></svg>
                      </button>
                    </div>

                    <div style={{ fontSize: '15px', color: 'var(--text-main)', lineHeight: 1.5, whiteSpace: 'pre-wrap', marginBottom: '12px', paddingLeft: '2px' }}>{post.bio?.trim()}</div>
