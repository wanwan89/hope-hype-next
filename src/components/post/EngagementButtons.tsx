  ...
  return (
    <div className="engagement-group" onClick={(e) => e.stopPropagation()}>
      ...
      >
        ...
        <span className="like-count" style={{ color: myLikedPosts.has(postId) ? '#ff2e63' : 'var(--text-main)' }}>{counts[postId]?.likes || 0}</span>
      </button>
      ...
      >
        ...
        <span className="comment-count" style={{ color: 'var(--text-main)' }}>{counts[postId]?.comments || 0}</span>
      </button>
    </div>
  );
};
