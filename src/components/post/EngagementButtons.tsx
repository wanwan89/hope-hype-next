...
type EngagementButtonsProps = {
  postId: string;
  creatorId: string; 
  // Tetap sediakan properti comments
  counts: Record<string, { likes: number; comments?: number; tanggapan?: number; reposts: number; saves: number }>;
...
const EngagementButtons: React.FC<EngagementButtonsProps> = ({
  ...
  const handleOpenComment = (e: React.MouseEvent) => {
    ...
    e.stopPropagation(); // Memastikan event tidak bocor ke card (parent)
  ...
  return (
    ...
    >
      ...
      <button
        ...
        style={{ background: 'transparent', border: 'none', display: 'flex', alignItems: 'center', gap: '6px', cursor: 'pointer', padding: 0 }}
        onClick={(e) => {
          e.preventDefault();
          e.stopPropagation();
          handleSave(postId);
      ...
      >
        ...
        <span className="save-count" style={{ color: 'var(--text-main)', fontSize: '13px', fontWeight: 600 }}>
      ...
      >
        <svg 
          ...
          fill="currentColor" 
          style={{ color: myRepostedPosts.has(postId) ? "#1f3cff" : "var(--text-main)", transition: '0.2s' }}
        >
          <path d="M4.5 3.88l4.432 4.14-1.364 1.46L5.5 7.55V16c0 1.1.896 2 2 2H13v2H7.5c-2.209 0-4-1.79-4-4V7.55L1.432 9.48.068 8.02 4.5 3.88zM16.5 6H11V4h5.5c2.209 0 4 1.79 4 4v8.45l2.068-1.93 1.364 1.46-4.432 4.14-4.432-4.14 1.364-1.46 2.068 1.93V8c0-1.1-.896-2-2-2z"/>
        </svg>
        <span className="repost-count" style={{ color: 'var(--text-main)', fontSize: '13px', fontWeight: 600 }}>
      ...
      >
        <svg 
          ...
          fill="currentColor" 
          style={{ color: myLikedPosts.has(postId) ? '#ff2e63' : 'var(--text-main)', transition: '0.2s' }}
        >
        ...
        <span className="like-count" style={{ color: 'var(--text-main)', fontSize: '13px', fontWeight: 600 }}>
      ...
      >
        <svg viewBox="0 0 24 24" width="20" height="20" className="icon" fill="currentColor" style={{ color: 'var(--text-main)' }}>
