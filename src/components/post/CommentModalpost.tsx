...
export default function CommentModalpost() {
  ...
  // 🔥 LOGIC GIFT BARU (Dipindah dari Gallery) 🔥
  const handleGiftClick = () => {
    if (!currentCreatorId) return;
    // Trigger event custom buat buka komponen Gift
  ...
  return (
    <div id="commentModal" className={isActive ? "active" : ""}>
      <div className="comment-box">
        {/* 🔥 FIX: Header diubah buat nampung tombol Gift 🔥 */}
        <div className="comment-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <span>Komentar</span>
          <div style={{ display: 'flex', gap: '15px', alignItems: 'center' }}>
            <button className="modal-gift-btn" onClick={handleGiftClick} style={{ background: 'none', border: 'none', color: '#f59e0b', cursor: 'pointer', display: 'flex', padding: 0 }}>
            ...
            <button className="comment-close" onClick={closeModal} style={{ position: 'static', margin: 0, padding: 0 }}>&times;</button>
          </div>
        </div>
