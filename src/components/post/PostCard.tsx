...
type Props = {
  ...
  showTopComment?: boolean;   // <-- penting
  tanggapanLabel?: string;    // <-- penting
};
...
export default function PostCardText({
  ...
  router, t, onTanggapanClick,
  showTopComment = true,      // default true (hanya muncul di halaman utama)
  tanggapanLabel,
}: Props) {
  ...
  // Ambil 1 tanggapan teratas HANYA jika showTopComment true
  useEffect(() => {
    let isMounted = true;
    if (!showTopComment) return; // <-- pengecekan
  ...
  }, [postIdStr, showTopComment]); // dependensi showTopComment
  ...
  return (
    ...
    >
      ...
      {post.audio_src && (
        <>
          <audio ref={audioRef} src={post.audio_src} className="post-audio-element" loop playsInline autoPlay muted={isGloballyMuted} style={{ width: '1px', height: '1px', position: 'absolute', opacity: 0 }} />
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginTop: '10px' }}>
            <MusicMarquee post={post} isOverlay={false} mediaRef={audioRef} />
            <button className="btn-press" onClick={(e) => { e.stopPropagation(); toggleMute(e); if (audioRef.current && isGloballyMuted) { audioRef.current.muted = false; audioRef.current.play().catch(() => {}); } }} style={{ background: 'var(--bg-secondary)', border: '1px solid var(--border-card)', color: 'var(--text-main)', width: '32px', height: '32px', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      ...
      {/* 🔥 HANYA TAMPILKAN JIKA showTopComment TRUE */}
