...
const PostCard: React.FC<PostCardProps> = ({
  ...
  // 🔥 State Baru untuk Kontrol Video & Progress Bar
  const [isPlaying, setIsPlaying] = useState(false);
  const [playPauseIcon, setPlayPauseIcon] = useState<'play' | 'pause' | null>(null);
  const [videoProgress, setVideoProgress] = useState(0);

  // --- 3. Observer video/audio yang dioptimalkan ---
  useEffect(() => {
  ...
  // --- 6. Deteksi tombol "Lihat Selengkapnya" ---
  useEffect(() => {
  ...
  // 🔥 8. Style card - FIX CELAH PINGGIRAN EDGE-TO-EDGE
  const cardStyle: React.CSSProperties = useMemo(() => ({
    overflow: actuallyExpanded ? 'visible' : 'hidden',
    background: 'var(--bg-main)',
    width: '100vw', // Paksa full width layar
    maxWidth: '100%',
    margin: '0 0 12px 0',
    padding: 0, // Tidak ada padding sama sekali untuk card utama
    border: 'none', // Menghilangkan border pinggir
    borderTop: '1px solid var(--border-card)',
    borderBottom: '1px solid var(--border-card)',
    borderRadius: 0, // Sudut tidak melengkung
  ...
  return (
    <div key={postIdStr} id={`post-${postIdStr}`} data-postid={postIdStr} className="card" ref={cardRef} style={cardStyle} onClick={(e) => { if (!photoList.length && !isVideoPost) handleMediaClick(e, postIdStr, creatorIdStr); }}>
      ...
      {(photoList.length > 0 || isVideoPost) ? (
        <>
          {/* Container Slider Disesuaikan agar Full Width */}
          <div className="slider" style={{ position: 'relative', width: '100%', overflow: 'hidden' }}>
              ...
              {isVideoPost ? (
                ...
                <div className="carousel-item" onClick={handleVideoTap} style={{ aspectRatio: '4/5', width: '100%', position: 'relative', background: '#000', cursor: 'pointer' }}>
                  ...
                  <video
                    ...
                    style={{ width: '100%', height: '100%', objectFit: 'cover', objectPosition: 'center', willChange: 'transform', opacity: videoLoaded ? 1 : 0, transition: 'opacity 0.3s' }}
                  />

                  {/* 🔥 Progress Bar Video di Atas Mute Button */}
                  <div style={{ position: 'absolute', bottom: 0, left: 0, width: '100%', height: '4px', background: 'rgba(255,255,255,0.2)', zIndex: 5 }}>
                    ...
                    <input
                      ...
                      onChange={handleSeek}
                      onClick={(e) => e.stopPropagation()} // Supaya ga play/pause
            ...
            {/* Mute dipindah ke Kanan Bawah */}
            {(isVideoPost || post.audio_src) && (
              <button
                className="btn-press"
                onClick={(e) => { e.stopPropagation(); toggleMute(e); if (mediaRef.current && isGloballyMuted) { mediaRef.current.muted = false; mediaRef.current.play().catch(() => {}); } }}
                style={{ position: 'absolute', bottom: isVideoPost ? '16px' : '12px', right: '12px', zIndex: 6, background: 'rgba(0,0,0,0.5)', backdropFilter: 'blur(8px)', border: 'none', color: 'white', width: '28px', height: '28px', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer' }}
              >
                <span className="material-icons" style={{ fontSize: '16px' }}>{isGloballyMuted ? 'volume_off' : 'volume_up'}</span>
              </button>
            )}
          ...
          {/* Overlay informasi (Padding ditambahkan di sini, BUKAN di parent card) */}
