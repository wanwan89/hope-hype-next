...
const PostCard: React.FC<PostCardProps> = ({
  ...
  const cardRef = useRef<HTMLDivElement>(null); // Referensi kartu penuh
  ...
  // FIX AUDIO: Pantau pergerakan kartunya (cardRef), bukan elemen audionya yang 1px!
  useEffect(() => {
    ...
    }, { threshold: 0.5 }); // Sedikit diturunkan agar audio lebih mudah ke trigger
    
    observer.observe(card); // Kita pantau kartu postingannya
  ...
  return (
    <div key={postIdStr} id={`post-${postIdStr}`} data-postid={postIdStr} className="card"
      ref={cardRef} // Penting untuk auto-play
      style={cardStyle}
      ...
      {(photoList.length > 0 || isVideoPost) ? (
        ...
        <>
          ...
          {post.audio_src && (
            <>
              {/* FIX AUDIO: autoPlay ditambahkan. */}
              <audio
