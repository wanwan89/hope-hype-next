    ...
    <div className="destination-container">
        ...
        {[
          ...
          <label key={dest.id} className="dest-option" style={{ display: 'flex', alignItems: 'center', background: 'var(--bg-secondary)', padding: '15px', borderRadius: '12px', border: destination === dest.id ? '2px solid #1f3cff' : '2px solid transparent', cursor: 'pointer', transition: 'all 0.2s' }}>
            ...
            <div className="dest-content" style={{ display: 'flex', alignItems: 'center', width: '100%', gap: '15px', padding: 0, background: 'transparent', border: 'none' }}>
              ...
              <div className="dest-check" style={{ width: '20px', height: '20px', borderRadius: '50%', border: destination === dest.id ? '6px solid #1f3cff' : '2px solid var(--border-card)' }} />
            </div>
          </label>
        ))}
      </div>
      {destination === 'story' && (
        <div className="visibility-toggle" style={{ display: 'flex', gap: '8px', marginTop: '15px', background: 'var(--bg-secondary)', padding: '6px', borderRadius: '14px' }}>
          <button type="button" onClick={() => setVisibility('public')} style={btnStyle(visibility === 'public')}><span className="material-icons" style={{ fontSize: '16px' }}>public</span> Publik</button>
          <button type="button" onClick={() => setVisibility('followers')} style={btnStyle(visibility === 'followers')}><span className="material-icons" style={{ fontSize: '16px' }}>group</span> Hanya Followers</button>
        </div>
      )}
    </div>
  );
