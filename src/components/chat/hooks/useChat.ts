...
export function useChat() {
  ...
  const connectLiveKit = async (rName: string) => {
    // Letakkan full kode connectLiveKit kamu yang sudah benar di sini
  };

  const endCall = (silent = false) => {
    // Letakkan full kode endCall kamu yang sudah benar di sini
  };
  ...
  if (typingUser && (groupId || roomId === 'room-1')) {
    ...
    if (onlineUserNames.length === 0) displayStatus = `0 online`;
      ...
      displayStatus = `${onlineUserNames[0]} dan ${onlineUserNames.length - 1} lainnya online`;
    }
  } else {
  ...
  return {
    state: {
      router, t, groupId, roomId, currentUser, myProfile, headerInfo, targetId, messages,
      isLoading, inputValue, onlineCount, typingUser, replyTo, isStickerOpen, stickers,
      isImageModalOpen, isUploadingImg, msgOptions, editMessageId, isGroupSettingsOpen,
      groupModalTab, groupMembers, newGroupName, isOwner, isUpdatingGroup, inviteSearch,
      relation, isMicPressed, isRecording, cancelAnim, recordTime, audioLevel, callStatus,
      callData, isSelectionMode, selectedMessages, onlineUserNames, chatState, displayStatus, canSend
    },
    actions: {
      setInputValue, setReplyTo, setIsStickerOpen, setStickers, setIsImageModalOpen,
      setIsUploadingImg, setMsgOptions, setEditMessageId, setIsGroupSettingsOpen,
      setGroupModalTab, setNewGroupName, setInviteSearch, setIsSelectionMode,
      handleAddMember, updateGroupInfo, kickMember, handleGroupPhotoUpload, fetchMessages,
      sendMessage, handleTyping, handleTerimaRequest, handleTolakRequest, startVN, stopVN,
      handleMicTouchStart, handleMicTouchMove, handlePhotoClick, handleSendImageFromModal,
      handleSendClick, handleEditMessage, handleDeleteMessage, toggleSelectMessage,
      selectAllMessages, handleBulkDelete, startCall, connectLiveKit, endCall, fetchStickers
    },
    refs
  };
}
