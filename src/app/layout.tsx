...
export default function RootLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      ...
      if (session?.user) {
        ...
        if (data) {
          setMyProfile(data);
          if (fcmTokenRef.current) updatePushToken(session.user.id, fcmTokenRef.current);
        }
      }
    };
    fetchProfile();
        ...
        if (platform === 'android' || platform === 'ios') {
          ...
          if (platform === 'android') {
            await PushNotifications.createChannel({
              id: 'hype_high_channel', name: 'Panggilan & Chat HypeTalk', description: 'Channel penting', importance: 5, visibility: 1, sound: 'suara_panggilan', vibration: true,
            });
          }
          if (permPush.receive === 'granted') await PushNotifications.register();
          ...
          PushNotifications.addListener('pushNotificationActionPerformed', (action) => {
            ...
            if (data && data.roomId) {
              ...
              if (data.type === 'call') router.push(`/hypetalk/room?from=${targetUserId}&incomingCall=true`);
              else router.push(`/hypetalk/room?from=${targetUserId}`);
            }
          });
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      ...
      if (globalIncomingCall) ringtoneRef.current.play().catch(() => {});
      ...
      {!isOnline && (
        <div className="offline-global-overlay" style={{ position: 'fixed', inset: 0, display: 'flex', justifyContent: 'center', alignItems: 'center', background: 'rgba(0,0,0,0.6)', backdropFilter: 'blur(4px)', zIndex: 9999999 }}>
          <div className="offline-card" style={{ textAlign: 'center', padding: '2rem', borderRadius: '1.5rem', background: 'transparent', border: 'none', maxWidth: '320px', width: '90%', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
            <div style={{ width: '150px', height: '150px', marginBottom: '10px' }}><Lottie animationData={lostConnectionData} loop={true} autoplay={true} /></div>
    ...
    <html lang="id" suppressHydrationWarning>
      <head>
        ...
        <style>{`
          ...
          html, body, .layout-wrapper, .main-content { border-top: none !important; box-sizing: border-box; }
