...
// 🔥 FIX ERROR 1: JANGAN IMPORT INI SECARA STATIS DI ATAS 🔥
// import { SplashScreen } from '@capacitor/splash-screen'; // <-- DIHAPUS
...
// 🔥 FIX ERROR 2: UPDATE PATH IMPORT SESUAI LOKASI src/Components 🔥
    ...
    const hideNativeSplash = async () => {
      try {
        ...
        if (platform === 'android' || platform === 'ios') {
          // 🔥 FIX ERROR 1: PAKE DYNAMIC IMPORT DI SINI 🔥
          // Biar Next.js nggak nyari paket ini pas build di Vercel
    ...
    <html lang="id" suppressHydrationWarning>
      <head>
        ...
        <style>{`
          ...
          @keyframes pulseCallGlobal { 0% { box-shadow: 0 0 0 0 rgba(46, 204, 113, 0.6); } 70% { box-shadow: 0 0 0 10px rgba(46, 204, 113, 0);Oke Bree, ini kodingan `src/app/layout.tsx` yang udah **FULL FIX**. Langsung Copy-Paste total buat nimpa file lu yang lama:
...
// 🔥 FIX ERROR 2: UPDATE PATH IMPORT SESUAI LOKASI src/Components (Pake 'C' Gede) 🔥
import CustomSplash from '@/Components/CustomSplash';
  ...
  useEffect(() => {
    const hideNativeSplash = async () => {
      try {
        const platform = Capacitor.getPlatform();
        if (platform === 'android' || platform === 'ios') {
          // 🔥 FIX ERROR 1: PAKE DYNAMIC IMPORT DI SINI biar build Vercel aman 🔥
          const { SplashScreen } = await import('@capacitor/splash-screen');
          // Langsung sembunyikan splash bawaan pas web kita siap render!
          await SplashScreen.hide();
        }
  ...
  return (
    <html lang="id" suppressHydrationWarning>
      <head>
        ...
        <meta name="theme-color" content="#1f3cff" />
        <meta name="viewport" content="width=device-width, initial-scale=1, maximum-scale=5, viewport-fit=cover" />
        <link href="[https://fonts.googleapis.com/icon?family=Material+Icons&display=block](https://fonts.googleapis.com/icon?family=Material+Icons&display=block)" rel="stylesheet" />
        <link href="[https://fonts.googleapis.com/css2?family=Poppins:wght@300;400;600;700;800&display=swap](https://fonts.googleapis.com/css2?family=Poppins:wght@300;400;600;700;800&display=swap)" rel="stylesheet" />
        <style>{`
          body { background-color: var(--bg-main); }
          @keyframes slideDownGlobal { from { transform: translate(-50%, -120%); opacity: 0; } to { transform: translate(-50%, 0); opacity: 1; } }
          .global-call-avatar { width: 45px; height: 45px; border-radius: 50%; object-fit: cover; border: 2px solid #2ecc71; animation: pulseCallGlobal 1.5s infinite; }
      ...
      <body className={`antialiased ${isVoicePage ? 'in-voice-room' : 'in-home-app'}`}>
        ...
        <Script src="[https://cdn.jsdelivr.net/npm/eruda](https://cdn.jsdelivr.net/npm/eruda)" strategy="lazyOnload" onLoad={() => { if (typeof window !== 'undefined' && (window as any).eruda) (window as any).eruda.init(); }} />
