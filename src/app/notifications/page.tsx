...
export default function NotificationsPage() {
  ...
  const loadFriendStories = async (followingIds: Set<string>) => {
    ...
    try {
      ...
      (stories || []).forEach((s) => {
        storyMap.set(s.creator_id, s.id);
      });
  ...
  const loadNotifications = async (userId: string) => {
    ...
    try {
          ...
          finalLikesNotifs.push(
            ...likes.map((l: any) => ({
              ...
              actor: profilesMap[l.user_id] || {
                username: l.user_id,
                avatar_url: '/asets/png/profile.webp',
              },
      ...
      const formattedCommentLikes = commentLikesData.map((cl: any) => {
        ...
        return {
          id: `comment_like-${cl.id}`,
          type: 'comment_like',
          post_id: relatedComment?.post_id,
          actor_id: cl.user_id,
          created_at: cl.created_at,
          is_read: true,
          actor: profilesMap[cl.user_id] || {
            username: cl.user_id,
            avatar_url: '/asets/png/profile.webp',
          },
  ...
  const getIconAndColor = (type: string) => {
    switch (type) {
      ...
      case 'reply':
        return { icon: 'reply', color: '#10b981' };
      case 'comment_like':
        return { icon: 'favorite', color: '#ff2e63' };
      case 'repost':
