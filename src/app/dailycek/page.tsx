...
// SVG Coin Component
const CoinIcon = ({ size = 16, style }: { size?: number; style?: React.CSSProperties }) => (
  <svg
  ...
  const secureUpdate = async (updateObj: any, history: any) => {
    try {
      const { error: pErr } = await supabase
        .from('profiles')
        .update({ ...updateObj, device_id: visitorId })
        .eq('id', profile.id);
      if (pErr) throw pErr;
      ...
      await supabase.from('coin_history').insert([
        {
          ...
          amount: 200,
          description: 'Memakai kode undangan',
        },
        {
          user_id: friend.id,
          type: 'masuk',
          ...
          amount: 500,
          description: 'Teman memakai kodemu',
        },
      ]);
  ...
  if (showSecurity)
    return (
      <div className="security-overlay">
        <span className="material-icons" style={{ fontSize: '64px', color: '#ff4757' }}>
          shield
        </span>
      ...
      <div className="mission-container">
        <div className="sticky-top-wrapper">
          <header className="mission-header">
            ...
            {/* Header coin dengan SVG */}
            <div className="header-coin">
              <CoinIcon size={14} style={{ marginRight: 6 }} />
              <span style={{ color: '#f59e0b', fontSize: '15px', lineHeight: 1 }}>
          ...
          <div className="exchange-card-wrapper">
            <div className="mission-card exchange">
              <div style={{ display: 'flex', alignItems: 'center', gap: '15px' }}>
                <div className="m-icon-box" style={{ background: '#38bdf8' }}>
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <p style={{ fontSize: '0.7rem', marginTop: 4 }}>
                    Saldo Hope: <b style={{ color: '#38bdf8' }}>{profile?.coins || 0}</b>
                  </p>
                </div>
                <button
                  className="m-btn btn-klaim"
        ...
        <div className="mission-scroll-area">
              ...
              {REWARDS.map((rew, i) => {
                ...
                return (
                  <div key={i} className={`day-card ${isDone ? 'done' : ''} ${isActive ? 'active' : ''}`}>
                    ...
                    <span className="material-icons" style={{ color: '#f59e0b', margin: '5px 0' }}>
                      stars
                    </span>
          ...
          <div className="mission-list">
            {!profile?.apk_downloaded && (
              <div className="mission-item">
                <div className="m-icon-box" style={{ background: '#10b981' }}>
                ...
                <button className="m-btn btn-klaim" onClick={handlePwaInstall}>
                  Instal
                </button>
              </div>
            )}
            ...
            <div className="mission-item" style={{ flexWrap: 'wrap' }}>
              <div className="m-icon-box" style={{ background: '#f97316' }}>
              ...
              <div className="m-info">
                ...
                {!profile?.referred_by ? (
                  ...
                  <span className="progress-text" style={{ color: '#10b981' }}>
                    ✓ Berhasil diklaim
                  </span>
                )}
              </div>
                ...
                <div className="mission-item" key={key}>
                  ...
                  <div className="m-info">
                    <h4 className="m-title">
                      {key.toUpperCase()} {m.target}X
                    </h4>
                    <div className="m-reward">+{m.reward} Koin</div>
                    <div className="progress-bar">
                      <div
                        className="progress-fill"
                        style={{ width: `${Math.min((current / m.target) * 100, 100)}%` }}
                      />
                    </div>
                  </div>
                  <button
                    ...
                    onClick={() => {
                      ...
                      if (canClaim)
                        secureUpdate(
                          { mission_coins: profile.mission_coins + m.reward },
                          {
                            type: 'masuk',
          ...
          <div className="mission-list" style={{ marginBottom: '2rem' }}>
            <div className="mission-item">
              <div className="m-icon-box" style={{ background: '#ef4444' }}>
              ...
              <div className="m-info">
                ...
                <span className="progress-text">
                  Kode:{' '}
                  <b style={{ color: 'var(--primary)', fontFamily: 'monospace' }}>
                    {profile?.invite_code || '...'}
                  </b>
                </span>
              </div>
              <button className="m-btn" onClick={handleShareLink}>
                Bagikan
              </button>
            </div>
          </div>
