...
export default function VipPage() {
  ...
  // Helper untuk manfaat VIP (tetap pakai translasi)
  ...
  // Payment handler (sama seperti sebelumnya, hanya disesuaikan notifikasi)
  ...
  const handleBuyVip = (role: string, price: number, name: string) => {
    ...
    processPayment(
      'https://hqetnqnvmdxdgfnnluew.supabase.co/functions/v1/pay-premium',
      payload,
      `vip-${role}`
    );
  };

  const openInfoModal = (title: string, role: string) => {
    // Gunakan getUserBadge untuk menghasilkan ikon yang sesuai dengan tema
    const badgeHtml = getUserBadge(role);
    setInfoModal({ isOpen: true, title, role, iconHtml: badgeHtml });
  };
    ...
    <div className="vip-page-container">
      ...
      <section className="section-container">
        <p className="section-desc">
          {t(
            'vip_desc',
            'Tingkatkan profil kamu ke level selanjutnya. Pilih paket VIP di bawah ini dan nikmati berbagai fitur eksklusif khusus buat kamu.'
          )}
        </p>
        ...
        <div className="product-list">
          ...
          <div className="product-card">
            <div className="product-info-wrapper">
              <div
                ...
                dangerouslySetInnerHTML={{ __html: getUserBadge('verified') }}
              />
              <div className="product-text">
                <span className="p-name">
                  ...
                  <svg
                    className="info-icon"
                    onClick={() => openInfoModal('Verified Badge', 'verified')}
                    viewBox="0 0 24 24"
                    width="18"
                    height="18"
                    stroke="currentColor"
            ...
            <button
              ...
              disabled={processingId === 'vip-verified'}
              onClick={() => handleBuyVip('verified', 30000, 'Verified Badge')}
            >
          ...
          <div className="product-card">
            <div className="product-info-wrapper">
              <div
                className="product-icon"
                dangerouslySetInnerHTML={{ __html: getUserBadge('crown1') }}
              />
              <div className="product-text">
                <span className="p-name">
                  Crown Bronze
                  <svg
                    className="info-icon"
                    onClick={() => openInfoModal('Crown Bronze', 'crown1')}
                    viewBox="0 0 24 24"
                    width="18"
                    height="18"
                    stroke="currentColor"
            ...
            <button
              className="buy-now-btn"
              disabled={processingId === 'vip-crown1'}
              onClick={() => handleBuyVip('crown1', 14999, 'Crown Bronze')}
            >
              {processingId === 'vip-crown1'
          ...
          <div className="product-card">
            <div className="product-info-wrapper">
              <div
                className="product-icon"
                dangerouslySetInnerHTML={{ __html: getUserBadge('crown2') }}
              />
              <div className="product-text">
                <span className="p-name">
                  Crown Silver
                  <svg
                    className="info-icon"
                    onClick={() => openInfoModal('Crown Silver', 'crown2')}
                    viewBox="0 0 24 24"
                    width="18"
                    height="18"
                    stroke="currentColor"
            ...
            <button
              className="buy-now-btn"
              disabled={processingId === 'vip-crown2'}
              onClick={() => handleBuyVip('crown2', 20000, 'Crown Silver')}
            >
              {processingId === 'vip-crown2'
                ? t('processing', 'Memproses...')
                : t('buy', 'Beli')}
            </button>
          </div>
          ...
          <div className="product-card gold-card">
            <div className="product-info-wrapper">
              <div
                className="product-icon"
                dangerouslySetInnerHTML={{ __html: getUserBadge('crown3') }}
              />
              <div className="product-text">
                <span className="p-name">
                  Crown Gold
                  <svg
                    className="info-icon"
                    onClick={() => openInfoModal('Crown Gold', 'crown3')}
                    viewBox="0 0 24 24"
                    width="18"
                    height="18"
                    stroke="currentColor"
            ...
            <button
              className="buy-now-btn"
              disabled={processingId === 'vip-crown3'}
              onClick={() => handleBuyVip('crown3', 49999, 'Crown Gold')}
            >
              {processingId === 'vip-crown3'
                ? t('processing', 'Memproses...')
                : t('buy', 'Beli')}
            </button>
          </div>
      ...
      {/* INFO MODAL */}
      <div
        className={`modal-overlay ${infoModal.isOpen ? 'active' : ''}`}
      ...
      >
        <div className="info-modal-content" onClick={(e) => e.stopPropagation()}>
          ...
          <div
            className="info-icon-showcase"
            dangerouslySetInnerHTML={{ __html: infoModal.iconHtml }}
          />
          <h3 className="premium-title">{infoModal.title}</h3>
          <div
            className="premium-benefits-list"
            dangerouslySetInnerHTML={{ __html: getVipBenefitsHtml(infoModal.role) }}
          />
          <button
            className="premium-btn"
            onClick={() => setInfoModal({ ...infoModal, isOpen: false })}
          >
            {t('understand', 'Mengerti')}
          </button>
        </div>
      </div>
    </div>
