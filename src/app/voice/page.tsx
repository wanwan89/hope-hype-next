...
const overlay = document.getElementById('room-drawer-overlay'); // 🔥 Pakai ID unik
    ...
    async function fetchTopGifters() {
        try {
            // 🔥 FIX: Pakai fungsi getRoomLeaderboard biar ngitung kado di room INI aja 🔥
            const topUsers = await getRoomLeaderboard();

            const container = document.getElementById('top-gifters-container');
            if (!container) return; 

            // Kalau belum ada yang nyawer di panggung ini, HILANGKAN!
            ...
            // Kalau ada yang nyawer, pastikan wadahnya kelihatan lagi
            ...
            // Ambil maksimal 3 orang top buat mejeng di header
            topUsers.slice(0, 3).forEach((user: any, index: number) => {
                ...
                html += `
                    <img src="${user.avatar_url || 'asets/png/profile.png'}" 
                         title="${user.username} (Total: ${user.room_total} koin)"
                         style="width: 28px; height: 28px; border-radius: 50%; object-fit: cover; 
                                border: 2px solid ${bingkaiWarna[index] || '#555'}; 
