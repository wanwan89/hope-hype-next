...
function VoiceRoomContent() {
  ...
  const lkInstance = useRef<Room | null>(null); // Ref untuk akses cleanup

  // --- FIX: LIFECYCLE MANAGEMENT ---
  useEffect(() => {
    // 1. Tambahkan class lock scroll ke body
    ...
    // 2. CLEANUP FUNCTION (Dijalankan saat pindah halaman/keluar)
    return () => {
      // Lepas lock scroll agar halaman lain bisa di-scroll normal
      document.body.classList.remove('room-active');
      
      // Putus koneksi LiveKit jika ada
      if (lkInstance.current) {
        lkInstance.current.disconnect();
      }
    };
  }, [CURRENT_ROOM_ID]);
  ...
  const initLiveKit = async (userId: string, username: string) => {
    try {
      ...
      lkInstance.current = lkRoom; // Simpan ke ref untuk cleanup
  ...
  const keluarRoom = async () => {
    if (isOwner && confirm("Tutup panggung?")) {
  ...
  const sendGift = async (giftName: string, harga: number, giftId: number) => {
    ...
    if (selectedTarget.profile_id === currentUser.id) return showNotif("Jangan nyawer diri sendiri!", "warning");
    if (currentUser.coins < harga) return showNotif("Koin kurang!", "error");
  ...
  return (
    <div className="app-container">
      ...
      <header className="main-header">
        <div className="header-left">
          ...
          <div className="online-status">
            ...
            <span><b id="online-count">{onlineCount}</b> orang</span>
          </div>
        </div>
        <div className="header-right" style={{ display: 'flex', alignItems: 'center' }}>
          <div className="top-gifters-wrapper" onClick={openLeaderboard}>
            <span style={{ fontSize: '11px', color: '#FFD700', fontWeight: 800, marginRight: '6px' }}>🏆 TOP</span>
            <div style={{ display: 'flex', alignItems: 'center' }}>
              {topGifters.map((user, i) => (
                <img key={i} src={user.avatar_url || '/asets/png/profile.png'} style={{ width: '28px', height: '28px', borderRadius: '50%', border: `2px solid ${i===0?'#FFD700':i===1?'#C0C0C0':'#CD7F32'}`, marginLeft: i===0?'0':'-12px', zIndex: 3-i }} />
              ))}
            </div>
          </div>
              ...
              {user ? (
                <>
                  <div className={`avatar ${isSpeaking ? 'speaking' : ''}`} onClick={() => isMe ? setActiveModal('turun') : (isOwner && !isMe && kickUser(slot.profile_id, user.username))}>
                    ...
                    {user.mic_off && <div className="mute-badge"><span className="material-icons" style={{ color: '#e74c3c', fontSize: '14px' }}>mic_off</span></div>}
                  </div>
                  <span className="name-label" style={{ color: getLevelStyle(user.level).color }}>{user.username}{getUserBadge(user.role)}</span>
                </>
              ) : (
                <div className="avatar" onClick={() => handleNaikStage(i)}><span className="material-icons">add</span></div>
              )}
            </div>
          )
        })}
      ...
      <div id="chat-box" className="chat-display">
        <div className="msg system">Gunakan bahasa sopan dan saling menghargai ya!</div>
        {chatMessages.map(msg => (
           msg.username === "SISTEM_GIFT" ? <div key={msg.id} className="msg system-gift">🎁 {msg.text}</div> :
           <div key={msg.id} className="msg"><span className="user" style={{ color: getLevelStyle(msg.level).color }}>{msg.username}</span>: {msg.text}</div>
        ))}
        <div ref={chatEndRef} />
      </div>
      ...
      <footer className="footer-controls">
        ...
        <form className="input-wrapper" onSubmit={kirimKomentar}>
          <input type="text" id="chat-input" value={chatInput} onChange={e => setChatInput(e.target.value)} placeholder="Ketik komentar..." />
          <button type="submit" className="btn-send"><span className="material-icons">send</span></button>
        </form>
      </footer>

      {/* GIFT ANIM */}
      <div id="gift-anim-overlay" className={giftAnim.show ? 'show' : ''}>
         <img id="gift-anim-img" src={`/asets/gif/giftvid${giftAnim.giftId}.gif?t=${Date.now()}`} />
      </div>
      ...
      <div className={`gift-drawer ${isDrawerOpen ? 'open' : ''}`}>
        ...
        <div className="drawer-header">
          ...
          <div className="coin-panel"><span>{currentUser?.coins?.toLocaleString() || 0}</span></div>
        </div>
      ...
      <div className={`sidebar ${isSidebarOpen ? 'active' : ''}`}>
        ...
        <div className="sidebar-menu">
          <a onClick={() => router.push('/data')}><div className="menu-icon-box"><span className="material-icons">person</span></div> Profil</a>
          <a onClick={toggleMic}><div className="menu-icon-box"><span className="material-icons">{isMicOn ? "mic" : "mic_off"}</span></div> {isMicOn ? "Mute" : "Unmute"}</a>
          <a onClick={keluarRoom}><div className="menu-icon-box"><span className="material-icons">logout</span></div> Keluar</a>
        </div>
      </div>
      ...
      <div className={`modal-overlay ${activeModal ? 'show' : ''}`} onClick={() => setActiveModal(null)}>
        ...
        {activeModal === 'leaderboard' && (
           <div className="modal-box" onClick={e => e.stopPropagation()}>
              <h3 className="gold-title">🏆 TOP SULTAN</h3>
              <div className="leaderboard-list">
                 {leaderboard.map((u, i) => (
                    <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '10px' }}>
                       <b>{i+1}</b>
                       <img src={u.avatar_url || '/asets/png/profile.png'} style={{ width: '35px', borderRadius: '50%' }} />
                       <div style={{ flex: 1 }}>{u.username}</div>
                       <div style={{ color: '#FFD700' }}>{u.room_total.toLocaleString()}</div>
                    </div>
                 ))}
              </div>
           </div>
