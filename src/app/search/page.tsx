...
// Diperbarui: minHeight diubah jadi 60vh agar mengisi sisa layar saat loading
const FullSpinner = () => (
...
function SearchContent() {
  ...
  // KEMBALIKAN isLoading untuk mencegah layout "gepeng"
  ...
  useEffect(() => {
    // Aktifkan loading tiap kali query berubah (tekan enter)
    setIsLoading(true);
    ...
    try {
      ...
      if (!isHashtag) {
        ...
        if (userData) {
          ...
          setUsers(userData.map(u => ({ ...u, recentPosts: userPosts ? userPosts.filter(p => p.creator_id === u.id).slice(0, 3) : [] })));
        }
      } else {
        setUsers([]);
      }
  ...
  return (
    // FIX PENTING: minHeight diubah dari '100%' menjadi '100vh'
    <div style={{ minHeight: '100vh', background: 'var(--bg-main)', paddingBottom: '80px', maxWidth: '600px', margin: '0 auto', position: 'relative' }}>
      ...
      <div style={{ padding: '20px' }}>
        ...
        {/* Render Spinner Jika Sedang Fetch Data */}
        {isLoading ? (
          <FullSpinner />
        ) : (
          <>
                ...
                {recentSearches.length > 0 && (
                  <div>
                    <h3 style={{ fontSize: '14px', color: 'var(--text-main)', marginBottom: '12px', fontWeight: 800, display: 'flex', alignItems: 'center', gap: '6px' }}>
                      <span className="material-icons" style={{ fontSize: '18px' }}>history</span> RIWAYAT PENCARIAN
                    </h3>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                      {recentSearches.slice(0, showAllHistory ? recentSearches.length : 3).map((historyItem, idx) => (
                        <div key={idx} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 15px', background: 'var(--bg-secondary)', borderRadius: '12px', cursor: 'pointer' }} onClick={() => handleExecuteSearch(historyItem)}>
                          <div style={{ display: 'flex', alignItems: 'center', gap: '10px', color: 'var(--text-main)', fontSize: '14px' }}>
                            <span className="material-icons" style={{ fontSize: '16px', color: 'var(--text-muted)' }}>schedule</span>{historyItem}
                          </div>
                          <button onClick={(e) => deleteHistoryItem(e, historyItem)} style={{ background: 'none', border: 'none', color: 'var(--text-muted)', cursor: 'pointer' }}>
                    ...
                    {recentSearches.length > 3 && (
                      <button onClick={() => setShowAllHistory(!showAllHistory)} style={{ marginTop: '10px', background: 'none', border: 'none', color: 'var(--primary)', fontSize: '13px', fontWeight: 600, cursor: 'pointer' }}>
                ...
                {categoryRecommendations.length > 0 && (
                  <div>
                    <h3 style={{ fontSize: '14px', color: 'var(--text-main)', marginBottom: '15px', fontWeight: 800, display: 'flex', alignItems: 'center', gap: '6px' }}>
                      <span className="material-icons" style={{ color: '#00d2ff', fontSize: '18px' }}>explore</span> REKOMENDASI PENCARIAN
                    </h3>
                      ...
                      {categoryRecommendations.map((post) => (
                        <div key={post.id} onClick={() => router.push(`/post?id=${post.id}`)} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px', background: 'var(--bg-card)', border: '1px solid var(--border-card)', borderRadius: '12px', cursor: 'pointer' }}>
                          <div style={{ flex: 1, paddingRight: '15px' }}>
                            ...
                            <div style={{ fontSize: '12px', color: 'var(--text-muted)', display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>Pencarian terkait dengan @{post.profiles?.username}</div>
                          </div>
                          <div style={{ width: '60px', height: '60px', borderRadius: '8px', overflow: 'hidden', position: 'relative', background: 'var(--bg-secondary)' }}>
      ...
      {showSupportPopup && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.7)', backdropFilter: 'blur(5px)', zIndex: 9999, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px' }}>
          <div style={{ background: 'var(--bg-card)', padding: '30px 20px', borderRadius: '24px', width: '100%', maxWidth: '400px', textAlign: 'center' }}>
            <div style={{ width: '150px', height: '150px', margin: '0 auto' }}><Lottie animationData={babyLottie} loop={true} /></div>
            <h2 style={{ fontSize: '20px', fontWeight: 800, color: 'var(--text-main)', marginBottom: '12px' }}>Kamu Tidak Sendirian</h2>
            <p style={{ fontSize: '14px', color: 'var(--text-muted)', marginBottom: '24px' }}>Apapun yang sedang kamu hadapi saat ini, ada orang yang peduli dan siap mendengarkan.</p>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
              <button onClick={() => { setShowSupportPopup(false); router.push('/hypetalk/room?from=b648ab89-32b7-494a-b858-ee186f918f90'); }} style={{ background: 'var(--primary-bg)', color: '#fff', border: 'none', padding: '14px', borderRadius: '16px', fontWeight: 700, cursor: 'pointer' }}>
                Cerita dengan kami
              </button>
              <button onClick={() => setShowSupportPopup(false)} style={{ background: 'transparent', color: 'var(--text-muted)', border: 'none', padding: '12px', borderRadius: '16px', fontWeight: 600, cursor: 'pointer' }}>Tutup</button>
            </div>
          </div>
        </div>
      )}
...
export default function SearchPage() {
  return (
    <Suspense fallback={
      // Fallback suspense kita jadikan 100vh agar jika tertrigger Next.js, layarnya tetap full
