    ...
    <div className="tg-modal-overlay" style={{ display: 'flex' }} onClick={onClose}>
      <div className="tg-modal-content" onClick={(e) => e.stopPropagation()}>
        <div className="modal-header">
          ...
          <button className="close-modal-btn" onClick={onBack}><span className="material-icons">arrow_back</span></button>
        </div>
        ...
        <div className="settings-row" style={{ marginTop: '10px' }}>
          ...
          <input type="checkbox" className="ios-toggle" checked={mutedChats.has(profile.id)} onChange={() => onToggleMute(profile.id)} />
        </div>
