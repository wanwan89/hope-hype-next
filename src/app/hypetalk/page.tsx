...
// Import dinamis Livekit tetap di sini
...
export default function HypetalkPage() {
  ...
  const pathname = usePathname(); // Untuk deteksi pindah halaman
  ...
  // --- UI STATES ---
  const [isSidebarOpen, setIsSidebarOpen] = useState(false); // State tunggal buat Sidebar
  ...
  // FIX: Tutup sidebar otomatis kalau pindah halaman
  useEffect(() => {
    closeSidebar();
  }, [pathname]);
  ...
  const openModal = (modalName: string) => {
    ...
    closeSidebar(); // Tutup sidebar pas buka modal
  };
            ...
            <div key={chat.id} className="tg-chat-item" onClick={() => router.push(chat.type === 'global' ? '/chat' : `/chat?from=${chat.id}`)}>
              ...
              <div className="tg-chat-info">
                <div className="tg-chat-top">
                  <h4 className="tg-name">{chat.name} <span dangerouslySetInnerHTML={{ __html: getUserBadge(chat.role) }} /></h4>
        ...
        <div className="custom-modal-overlay" style={{ display: 'flex' }} onClick={closeModal}>
          <div className="custom-modal-content" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="input-group">
              ...
              <input type="text" placeholder="ID teman..." />
            </div>
      ...
      {activeModal === 'doi-card' && foundDoi && (
        <div className="custom-modal-overlay" style={{ display: 'flex' }} onClick={closeModal}>
          <div className="custom-modal-content doi-result-card" onClick={(e) => e.stopPropagation()}>
            ...
            <button className="action-btn love-btn" onClick={() => router.push(`/chat?from=${foundDoi.id}`)}>Gas Chat</button>
          </div>
        </div>
      )}
